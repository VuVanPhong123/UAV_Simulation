...
FAKE_CLIENT = ROOT / "test" / "fake_sharded_worker_client.js"
...
def main():
    ...
    try:
        server_proc = start_process(["node", "index.js"], server_cwd)
        wait_process_alive(server_proc, 2, "server")
        pass_step("server started")

        worker_env = {
            "WORKER_MAX_DRONES": "3",
            "WORKER_SUPPORTS_SHARDING": "true",
        }
        worker_a_proc = start_process(
            [worker_python_executable(), "worker.py"],
            CORE_DIR,
            env={**worker_env, "WORKER_NAME": "local-shard-worker-a"}
        )
        worker_b_proc = start_process(
            [worker_python_executable(), "worker.py"],
            CORE_DIR,
            env={**worker_env, "WORKER_NAME": "local-shard-worker-b"}
        )
        wait_process_alive(worker_a_proc, 2, "worker A")
        wait_process_alive(worker_b_proc, 2, "worker B")
        pass_step("two workers started")
        ...
        if fake_proc.returncode != 0:
            fail_step("sharded pipeline test", f"fake frontend exited with code {fake_proc.returncode}")
            print("fake frontend output tail:")
            print(tail_text(fake_output, 50))
            terminate_process(worker_a_proc)
            terminate_process(worker_b_proc)
            terminate_process(server_proc)
            print("server output tail:")
            print(tail_text(collect_output_after_stop(server_proc), 50))
            print("worker A output tail:")
            print(tail_text(collect_output_after_stop(worker_a_proc), 50))
            print("worker B output tail:")
            print(tail_text(collect_output_after_stop(worker_b_proc), 50))
            return 1
        ...
        if fake_proc:
            ...
            fake_output = ""
        fail_step("sharded pipeline test", "fake frontend timed out")
        print("fake frontend output tail:")
        print(tail_text(fake_output, 50))
        terminate_process(worker_a_proc)
        terminate_process(worker_b_proc)
        ...
        fail_step("sharded pipeline test", str(exc))
        terminate_process(worker_a_proc)
        terminate_process(worker_b_proc)
        terminate_process(server_proc)
        print("server output tail:")
